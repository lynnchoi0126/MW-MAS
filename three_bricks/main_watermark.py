...
def get_args_parser():
    ...
    # prompts parameters
    # parser.add_argument('--prompt_path', type=str, default="data/WIT_data2.json")
    parser.add_argument('--prompt_path', type=str, default="data/WIT_data_0519-1.json")
    ...
    parser.add_argument('--output_dir', type=str, default='output/WIT_05')
    ...
    #                     help='pvalue threshold(acceptable FPR) for detection of watermark')
...
def main(args):
    ...
    model = AutoModelForCausalLM.from_pretrained(
        model_name,
        device_map= "cuda", #"cuda", #"auto",
        torch_dtype=torch.float16,
        max_memory={i: '16000MB' for i in range(args.ngpus)},
    ...
    with open(os.path.join(args.output_dir, f"results.jsonl"), "a") as f:
        for ii in range(start_point, len(prompts), args.batch_size): ## start_point부터 시작해서 batch_size만큼씩 프롬프트 가져오기
            # generate chunk
            time0 = time.time()
            chunk_size = min(args.batch_size, len(prompts) - ii) # batch size 혹은 마지막까지 남은 프롬프트 수 만큼
            results = generator.generate(
                prompts[ii:ii+chunk_size], # 시작점부터 청크 사이즈 만큼 프롬프트에 대해
    ...
    # evaluate
    results = load_results(json_path=os.path.join(args.output_dir, f"results.jsonl"), nsamples=args.nsamples, result_key="result") # 워터마킹된 텍스트 로드
    log_stats = []
    text_index = left if args.split is not None else 0
    detection_start = time.time()
    with open(os.path.join(args.output_dir, 'scores.jsonl'), 'w') as f:
        for text, text_orig in tqdm.tqdm(zip(results, results_orig)): # 워터마킹된 텍스트와 원본 텍스트에 대해
            ...
            if args.payload_max:
                if pvalues.size>0:
                    try:
                        ...
                        print(f"Error processing pvalues: {e}")
                else:
                    print(f"Empty pvalues encountered in text {text_index}")
                    print(f"gt: {text_orig}")
                    print(f"result: {text}")
                    continue
            ...
            num_tokens = [len(score_no_aggreg) for score_no_aggreg in scores_no_aggreg] # number of tokens in each text
        ...
        ### 워터마킹 안 된 텍스트에 대해서도 워터마크 탐지 진행
        # no_watermark_txt = load_results(json_path=args.prompt_path, nsamples=int(args.nsamples*2), result_key="output") ### edited
        no_watermark_txt = load_results(json_path=args.prompt_path, result_key="output") ### edited

        print('number of non-watermarked text before slicing: ', len(no_watermark_txt))
        print('start_point: ', start_point)
        # no_watermark_txt = no_watermark_txt[start_point+len(prompts):]   ################## edited
        print('number of watermarked text: ', len(results))
        print('number of non-watermarked text: ', len(no_watermark_txt))
        if args.split is not None:
            no_watermark_txt = no_watermark_txt[left:right]
        for text in tqdm.tqdm(no_watermark_txt):
            start_time = time.time()
            # compute watermark score
            if args.method_detect == "openainp":
                scores_no_aggreg, probs = detector.get_scores_by_t([text], scoring_method=args.scoring_method, payload_max=args.payload_max)
            ...
            if args.payload_max:
                if pvalues.size > 0:
                    try:
                        ...
                        print(f"Error processing pvalues: {pvalues}. Exception: {e}")
                        payloads = [0]
                        all_pvalues = [[1.0]]
                        pvalues = [1.0]
                        scores = [0.0]
                else:
                    print("Empty pvalues encountered.")
                    payloads = [0]
                    all_pvalues = [[1.0]]
                    pvalues = [1.0]
                    scores = [0.0]
        ...
        print(f"Detection time: {detection_end - detection_start}")

        ### AUC Score
        from sklearn.metrics import roc_auc_score
        # pavlue_threshold = args.pvalue_threshold if hasattr(args, 'pvalue_threshold') else 1e-5
        if args.method_detect in ["openai", "openaiz"]:
            pvalue_threshold = 1e-3
            predicted_labels = [1 if stat['pvalue'] <= pvalue_threshold else 0 for stat in log_stats]
            true_labels = [1] * len(results) + [0] * len(no_watermark_txt)
            auc_score = roc_auc_score(true_labels, predicted_labels)
            print(f"AUC Score (based on p-value <= {pvalue_threshold}): {auc_score}")
        elif args.method_detect in ["maryland", "marylandz"]:
            pvalue_threshold = 1e-5
            predicted_labels = [1 if stat['pvalue'] >= pvalue_threshold else 0 for stat in log_stats]
        # predicted_scores = [stat['pvalue'] for stat in log_stats]
            true_labels = [1] * len(results) + [0] * len(no_watermark_txt)
            auc_score = roc_auc_score(true_labels, predicted_labels)
            print(f"AUC Score (based on p-value >= {pvalue_threshold}): {auc_score}")
        # print(f"AUC Score (based on p-value): {auc_score}")

        # ### accuracy
        # accuracy = sum(predicted_labels) / len(predicted_labels)
        # print(f"Accuracy (based on p-value <= {pvalue_threshold}): {accuracy}")

        df = pd.DataFrame(log_stats)
        # df['log10_pvalue'] = np.log10(df['pvalue'])
        print('average detection time per sample: ', df['detection_time'].mean())
